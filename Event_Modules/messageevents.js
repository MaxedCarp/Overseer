		...
		return new Promise((resolve, reject) => {
			(async () => {
				...
				for (let i = test.length - 1; i >= 0; i--) {
					...
					*/
					let resembed = await EmbedCreator.Create(`Message${messages2.length > 1 ? "s **BULK**" : ""} Deleted in: <#${messages[0].channelId}>`, `${messages2.length} Message${messages2.length > 1 ? "s" : ""} Deleted`, false, messages[0].guild.name, guild.iconURL(), `${global.client.user.userName}`, `https://cdn.discordapp.com/avatars/${global.client.id}/${global.client.user.avatar}`, 0xFA042A, []);
					let obj = await global.srvcol.findOne({ "srv": message.guild.id });
					if (obj.delete === "none" || !obj)
						return;
					if (((guild.members.me).permissionsIn(obj.delete).has(PermissionFlagsBits.SendMessages) && (guild.members.me).permissionsIn(obj.delete).has(PermissionFlagsBits.ViewChannel)) || (guild.members.me).permissionsIn(obj.delete).has(PermissionFlagsBits.Administrator))
